...
import { RetractionForm, StageHome, StagePayment, StageRegistration } from './StageJourney';
...
export default function App() {
  ...
  if (route === '/stage') return <StageHome />;
  if (route === '/inscription-stage') return <StageRegistration />;
  if (route === '/paiement-stage') return <StagePayment />;
  if (route === '/retractation-formulaire') return <RetractionForm />;
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#F8FAFC]">
      ...
      <footer className="border-t border-slate-200 bg-white px-6 py-8 text-center text-sm text-slate-600">
        ...
        <nav className="mt-3 flex flex-wrap justify-center gap-x-5 gap-y-2">
          <a className="hover:text-blue-700 hover:underline" href="/stage">Stage Choc Prépa</a>
